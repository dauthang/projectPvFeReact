...
import MonneyAddOrEdit from './MonneyAddOrEdit.js';
import MoneyPopupImport from './MoneyPopupImport.js';
class MonneyList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            tableHead: ["STT", "Hành động", "Tên báo cáo", "Tiền sống", "Tiền tiết kiệm", "Tiền chơi", "Tháng báo cáo", "Năm báo cáo"],
            tableData: [],
            data: null,
    ...
    onSearch() {
        fetch("http://localhost:8080/moneyService/moneys")
            .then(res => res.json())
            .then(
                (result) => {
                    let data = [];
    ...
    delete(id) {
        const url = `http://localhost:8080/moneyService/delete/${id}`;
        axios
            .delete(url)
            .then(res => {
                this.onSearch();
    ...
    detail(id) {
        axios({
            url: `http://localhost:8080/moneyService/money/${id}`,
            method: 'get',
        }).then((response) => {
    ...
    render() {
        return (
            <div>
                {
                    this.state.show ? <MonneyAddOrEdit closePopUp={this.closePopUp} data={this.state.data} action={this.state.action} />
                        : <GridContainer>
                            <GridItem xs={12} sm={12} md={12}>
                                ...
                                this.state.showPopupImport ? (
                                    <MoneyPopupImport openPopup={this.state.showPopupImport} closePopupImport={this.closePopupImport}>

                                    </MoneyPopupImport>) : null
                            }
                            </GridItem>
                        </GridContainer>
                }
